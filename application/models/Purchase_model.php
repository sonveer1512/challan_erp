...

{
	...
	public function getPurchase(){
		...
		if($this->session->userdata('type') == "admin"){
			...
			$this->db->select('p.*,s.*,ar.*')
				 ...
				 ->where('p.purchase_type','Purchase Invoice');
	...
	*/
	...
	public function createReferenceNo(){
		...
		$query = $this->db->query("SELECT * FROM purchases WHERE purchase_type = 'Purchase Invoice' ORDER BY purchase_id DESC LIMIT 1");
  ...
  public function getpurchasesDails(){
