...
class Requirement_model extends CI_Model
{
	...
	*/
	public function createReferenceNo(){
		$query = $this->db->query("SELECT * FROM requirements ORDER BY quotation_id DESC LIMIT 1");
		$result = $query->result();
		return $result;
	}
	/* 
	...
	*/
	public function addModel($data){
		...
		if($this->db->insert('requirements',$data)){
	...
	*/
	public function checkProductInQuotation($quotation_id,$product_id){
		$sql = "select * from requirement_items where quotation_id = ? AND product_id = ?";
	...
	*/
	public function updateQuantity($quotation_id,$product_id,$warehouse_id,$quantity,$old_quantity,$data){
		...
		$this->db->where($where);
		$this->db->update('requirement_items',$data);
	}
	/* 
	...
	*/
	public function addQuotationItem($data,$product_id,$warehouse_id,$quantity){
		...
		if($this->db->insert('requirement_items',$data)){
			return true;
		}
		else{
			return false;
	...
	*/
	...
	public function getQuotationItems2($quotation_id){
		$this->db->select('si.*,wp.quantity as warehouses_quantity,p.product_id,p.code,p.name,p.unit,p.price,p.cost,p.hsn_sac_code')
				 ->from('requirement_items si')
	...
	public function getrequirementforpurchaseorder() {
		$this->db->select('si.*,wp.quantity as warehouses_quantity,p.product_id,p.code,p.name,p.unit,p.price, p.quantity as available_quantity, p.cost,p.hsn_sac_code')
				 ->from('requirement_items si')
				 ->join('requirements r','si.quotation_id = r.quotation_id','left')
				 ->join('products p','si.product_id = p.product_id','left')
				 ->join('warehouses_products wp','wp.product_id = p.product_id','left')
				 ->where('si.delete_status',0)
				 ->where('r.completed',0)
	...
	public function getproductrequirementforpurchaseorder($id) {
		$this->db->select('si.*,wp.warehouse_name')
				 ->from('requirement_items si')
				 ->join('requirements r','si.quotation_id = r.quotation_id')
				 ->join('products p','si.product_id = p.product_id')
				 ->join('warehouse wp','wp.warehouse_id = r.warehouse_id')
				 ->where('si.delete_status',0)
	...
	*/
	public function deleteQuotationItems($quotation_id,$product_id,$warehouse_id,$old_warehouse_id){
		...
		if($this->db->update('requirement_items',array('delete_status'=>1))){
			return true;
		}
		else{
			return false;
	...
	*/
	public function deleteModel($id){
		...
		if($this->db->update('quotation',array('delete_status'=>1))){
			$this->db->where('quotation_id',$id);
			$this->db->update('requirement_items',array('delete_status'=>1));
		}
		else{
			return FALSE;
		}
	...
	*/
	public function getDetails($id){
		...
		$result = $this->db->select('q.*')
						 ->from('requirements q')
						 ->join('warehouse w','w.warehouse_id = q.warehouse_id','left')
						 ->join('branch b','b.branch_id = w.branch_id','left')
						 ->join('biller bl','bl.biller_id =	q.biller_id','left')
	...
	*/
	public function getItems($id){
		return  $this->db->select('si.*,pr.name,pr.code,pr.hsn_sac_code,pr.unit')
						 ->from('requirement_items si')
						 ->join('requirements s','si.quotation_id = s.quotation_id')
