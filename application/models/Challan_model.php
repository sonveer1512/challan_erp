...
class Challan_model extends CI_Model
{
	...
	*/
	public function getQuotation(){
		$this->db->select('q.*,b.*,c.*, t.driver_name, t.id as tid, t.vehicle_number')
		         ->from('challan q')
		         ->join('biller b','q.biller_id=b.biller_id')
		         ->join('customer c','q.customer_id=c.customer_id')
		         ->join('transport_settings t','t.id=q.vehicle_id')
		         ->where('q.user',$this->session->userdata('user_id'))
		         ->where('q.delete_status',0)
		         ->order_by('q.date','desc');
		return $this->db->get()->result();
	}


	public function getQuotationtypewise($type){
		$this->db->select('q.*,b.*,c.*, t.driver_name, t.id as tid, t.vehicle_number')
		         ->from('challan q')
		         ->join('biller b','q.biller_id=b.biller_id')
		         ->join('customer c','q.customer_id=c.customer_id')
	...
	public function getsiteQuotation($type){
		$this->db->select('q.*,b.*,c.*, t.driver_name, t.id as tid, t.vehicle_number')
		         ->from('challan q')
		         ->join('biller b','q.biller_id=b.biller_id')
		         ->join('customer c','q.customer_id=c.customer_id')
  	...
  	public function gettranportQuotation($type){
		$this->db->select('q.*,b.*,c.*, t.driver_name, t.id as tid, t.vehicle_number')
		         ->from('challan q')
		         ->join('biller b','q.biller_id=b.biller_id')
		         ->join('customer c','q.customer_id=c.customer_id')
  	...
  	public function gettranportQuotationChallan(){
		$this->db->select('q.*,b.*,c.*, t.driver_name, t.id as tid, t.vehicle_number')
		         ->from('challan q')
		         ->join('biller b','q.biller_id=b.biller_id')
		         ->join('customer c','q.customer_id=c.customer_id')
	...
	public function getdashboardsiteQuotation(){
		$this->db->select('q.*,b.*,c.*, count(q.id) as count')
		         ->from('challan q')
		         ->join('biller b','q.biller_id=b.biller_id')
		         ->join('customer c','q.customer_id=c.customer_id')
		         ->join('transport_settings t','t.id=q.vehicle_id')
		         ->where('q.user',$this->session->userdata('user_id'))
		         ->where('q.type','outward')
		         ->where('q.delete_status',0)
		         ->order_by('q.date','desc')
		         ->group_by('q.dispatch_to');
		return $this->db->get()->result();
	}



	public function getdashboardtransportQuotation(){
		$this->db->select('q.*,b.*,c.*, count(q.id) as count, t.vehicle_number, t.driver_name')
		         ->from('challan q')
		         ->join('biller b','q.biller_id=b.biller_id')
		         ->join('customer c','q.customer_id=c.customer_id')
		         ->join('transport_settings t','t.id=q.vehicle_id')
		         ...
		         ->where('q.delete_status',0)
		         ->order_by('q.date','desc')
		         ->group_by('q.transporter');
		return $this->db->get()->result();
	}


	public function getQuotationshortItems($challan_id){
		$this->db->select('si.*,p.product_id,p.code,p.name,p.unit,p.price,p.cost,p.hsn_sac_code')
				 ->from('challan_items si')
				 ->join('products p','si.product_id = p.product_id')
				 ->where('si.challan_id',$challan_id)
				 ->where('si.quantity > si.received_quantity');
	...
	public function countgetQuotationshortItems($challan_id){
  ...
  public function werehouselist()
  {
	...
	*/
	public function createReferenceNo(){
		$query = $this->db->query("SELECT * FROM challan ORDER BY id DESC LIMIT 1");
		$result = $query->result();
		return $result;
	}
	/* 
	...
	*/
	public function addModel($data){
		...
		if($this->db->insert('challan',$data)){
	...
	*/
	public function checkProductInQuotation($ch_id){
		$sql = "select * from challan_items where id = ?";
		if($quantity = $this->db->query($sql,array($ch_id))->num_rows() > 0){

			$sql = "select * from challan_items where id = ? ";
			$quantity = $this->db->query($sql,array($ch_id));
	...
	*/
public function updateQuantity($ch_id,$proID,$warehouse_id,$rquan,$old_quantity,$data){
		
		$this->db->where("id",$ch_id);
		$this->db->update('challan_items',$data);
	}


	public function updateReceivedQuantity($ch_id,$data){
		$where = "id = $ch_id";
		$this->db->where($where);
		$this->db->update('challan_items',$data);
	}
  
  	public function updatechallanrent($ch_id,$data){
		$this->db->where('id',$ch_id);
		$this->db->update('challan_items',$data);
	}
  ...
  public function InWarehouse($product_id,$quantity,$warehouse_id){
		$sql = "select * from warehouses_products where product_id = ? AND warehouse_id = ?";
		$query = $this->db->query($sql,array($product_id,$warehouse_id));
	...
	*/
	public function addQuotationItem($data,$proID,$warehouse_id,$qty){

		if($this->db->insert('challan_items',$data)){
			return true;
		}
		else{
			return false;
	...
	*/
	public function getQuotationItems($challan_id,$warehouse_id){
	...
	*/
	public function editModel($id,$data){
		$this->db->where('id',$id);
		if($this->db->update('challan',$data)){
			return true;
		}
		else{
			return false;
	...
	*/
	public function deleteModel($id){
		$this->db->where('id',$id);
		if($this->db->update('challan',array('delete_status'=>1))){
			$this->db->where('challan_id',$id);
			$this->db->update('challan_items',array('delete_status'=>1));
		}
		else{
			return FALSE;
		}
	...
	*/
	public function getItems($id){
		return  $this->db->select('si.*,pr.name,pr.code,pr.hsn_sac_code,pr.unit')
						 ->from('challan_items si')
						 ->join('challan s','si.challan_id = s.id','left')
	...
	*/
	public function checkInvoice($id){
		return $this->db->select('q.id')->from('challan q')->join('invoice i','i.quotation_id = q.id')->where('q.id',$id)->get()->row();

	}
  public function get_transportrate($id){
  		 
  	$sql="SELECT * FROM `transport_pay` WHERE id = $id";    
