...
class Ion_auth_model extends CI_Model
{
	...
	**/
	...
	public function allReports(){
      ...
      //YEAR
        $year  ="SELECT COUNT(*) as item FROM challan q Join biller b on q.biller_id=b.biller_id JOIN customer c on q.customer_id=c.customer_id JOIN transport_settings t ON t.id=q.vehicle_id WHERE q.user = '1' AND q.delete_status = 0 AND YEAR(q.date) = YEAR(CURDATE())";
      	$yearquery = $this->db->query($year);
        $data['yearProduct'] = $yearquery->result_array();
		
        //WEEK
      $week = "SELECT COUNT(*) AS item FROM challan q Join biller b on q.biller_id=b.biller_id JOIN customer c on q.customer_id=c.customer_id JOIN transport_settings t ON t.id=q.vehicle_id WHERE q.user = 1 AND q.delete_status = 0 AND q.date >= curdate() - INTERVAL DAYOFWEEK(curdate())+6 DAY AND q.date < curdate() - INTERVAL DAYOFWEEK(curdate())-1 DAY";
      $weekquery = $this->db->query($week);
        $data['weekProduct'] = $weekquery->result_array();
      
      //MONTH
      $month = "SELECT COUNT(*) AS item FROM challan q Join biller b on q.biller_id=b.biller_id JOIN customer c on q.customer_id=c.customer_id JOIN transport_settings t ON t.id=q.vehicle_id WHERE q.user = '1' AND q.delete_status = 0 AND MONTH(q.date) = MONTH(CURRENT_DATE()) AND YEAR(q.date) = YEAR(CURRENT_DATE())";
      $mothquery = $this->db->query($month);
     $data['monthProduct'] = $mothquery->result_array();
      
      //all
      $all = "SELECT COUNT(*) AS item FROM challan q Join biller b on q.biller_id=b.biller_id JOIN customer c on q.customer_id=c.customer_id JOIN transport_settings t ON t.id=q.vehicle_id WHERE q.user = '1' AND q.delete_status = 0";
      $allquery = $this->db->query($all);
     $data['allProduct'] = $allquery->result_array();
      
        
      
      //today 
      $today = "SELECT COUNT(*) AS item FROM challan q Join biller b on q.biller_id=b.biller_id JOIN customer c on q.customer_id=c.customer_id JOIN transport_settings t ON t.id=q.vehicle_id WHERE q.user = '1' AND q.delete_status = 0 and q.date = date('d')";
      $todayquery = $this->db->query($today);
     $data['todayProduct'] = $todayquery->result_array();
		...
		$data['transport_setting'] = $this->db->query('SELECT count(*) as count FROM transport_settings')->row();
